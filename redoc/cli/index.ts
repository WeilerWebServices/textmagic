        ...
        const config: Options = {
            ...
            redocOptions: {
                theme: {
                    ...
                    spacing: {
                        unit: 2,
                        sectionHorizontal: ({spacing}: { spacing: any }) => spacing.unit * 16,
                        sectionVertical: ({spacing}: { spacing: any }) => spacing.unit * 2,
                    },
                    ...
                    colors: {
                        ...
                        responses: {
                            success: {
                                color: ({colors}: { colors: any }) => colors.success.main,
                                // backgroundColor: ({ colors }) => transparentize(0.9, colors.success.main),
                            },
                            error: {
                                color: ({colors}: { colors: any }) => colors.error.main,
...
async function serve(port: number, pathToSpec: string, options: Options = {}) {
    ...
    const server = createServer((request, response) => {
        ...
        if (request.url === '/redoc.standalone.js') {
            respondWithGzip(
                createReadStream('./bundles/redoc.standalone.js', 'utf8'),
                request,
                response,
                {
...
async function getPageHTML(
    ...
    if (ssr) {
        ...
        if (!cdn) {
            redocStandaloneSrc = readFileSync('./bundles/redoc.standalone.js');
        }
    }
